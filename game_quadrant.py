...
def colliding(pair):
    particle, particle2 = pair
    """checks if particles are overlaping & moving closer to one another to trigger bouncing & avoid agglomeration"""
    if (np.linalg.norm(particle.pos - particle2.pos) - (particle.size + particle2.size) >= 0 ):
        return False
    return np.inner(particle.speed - particle2.speed, particle.pos - particle2.pos) <= 0
...
def draw_environment(particles):
    ...
    particles1 = [p for p in particles if p.pos[0] < WIDTH / 2 and p.pos[1] < HEIGHT / 2]    
    particles2 = [p for p in particles if p.pos[0] < WIDTH / 2 and p.pos[1] >= HEIGHT / 2]    
    particles3 = [p for p in particles if p.pos[0] >= WIDTH / 2 and p.pos[1] < HEIGHT / 2]    
    particles4 = [p for p in particles if p.pos[0] >= WIDTH / 2 and p.pos[1] >= HEIGHT / 2]    

    pari = [particles1, particles2, particles3, particles4]
